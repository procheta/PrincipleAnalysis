...
sys.path.append('/users/psen/EAP-positional/')
...
def main():
    try:
        print(sys.argv[1])
        df1 = pd.read_csv("part11/"+sys.argv[1]+".csv",sep="\t",on_bad_lines='skip',index_col=False)  # adjust this path if needed
    ...
    pd.DataFrame.from_dict(edges, orient='index').to_csv('imp_edges_principle11/qwen_important_edges'+sys.argv[1]+".csv", index=True)
